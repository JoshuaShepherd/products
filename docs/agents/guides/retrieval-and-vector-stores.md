...
await client.vectorStores.files.upload_and_poll({         // Upload file
    vector_store_id: vector_store.id,
...
Vector stores are the containers that power semantic search for the Retrieval API and the [file search](/docs/guides/tools-file-search) tool. They store your files and their embeddings, enabling efficient similarity search.
...
Upload files to your vector store
...
```javascript
await client.vectorStores.files.upload_and_poll({
    vector_store_id: vector_store.id,
    file: fs.createReadStream("customer_policies.txt"),
});
```
...
```javascript
const vector_stores = await client.vectorStores.list();
```
...
```javascript
const vector_store = await client.vectorStores.retrieve(vector_store_id);
```
...
```javascript
const vector_store = await client.vectorStores.update(vector_store_id, {
    name: "Updated Support FAQ"
});
```
...
```javascript
await client.vectorStores.delete(vector_store_id);
```
