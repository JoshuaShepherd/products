...
[CUA sample app](https://github.com/openai/openai-cua-sample-app) - Examples of how to integrate the computer use tool in different environments
...
4. **Capture the updated screenshot**: After executing the action, capture the updated state of the environment as a screenshot.

5. **Repeat**: Send the screenshot back to the model as a `computer_call_output` to get the next action. Repeat these steps as long as you get a `computer_call` item in the response.

### 1. Send a request to the model

Send a request to the model with the computer use tool
...
screenshot_bytes = get_screenshot(instance)  # Your screenshot function
...
import { getScreenshot } from "./screenshot.js"; // Your screenshot function
