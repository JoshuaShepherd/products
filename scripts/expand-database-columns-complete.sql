-- Database Column Expansion Migration (Complete View-Safe)
-- Date: 2025-09-08
...
DROP VIEW IF EXISTS products_with_pictograms CASCADE;
DROP VIEW IF EXISTS product_column_info CASCADE;
...
-- Now recreate the essential views with updated column types
-- Recreate the products_with_category view
...
-- Recreate the products_with_pictograms view
CREATE VIEW products_with_pictograms AS
SELECT 
    p.*,
    ARRAY_AGG(pic.url ORDER BY pp.sort_order) as related_pictogram_urls,
...
VALUES ('expand_product_columns_complete_2025_09_08', CURRENT_TIMESTAMP)
...
SELECT 'Column expansion migration completed successfully!' as status,
...
WHERE table_name = 'products' 
  AND table_schema = 'public'
  AND column_name IN ('shelf_life', 'voc_data', 'proper_shipping_name', 'emergency_response_guide', 'un_number', 'subtitle_1', 'subtitle_2', 'used_by_date', 'mot_de_signalement')
